...
[data-theme="dark"] {{
  --bg: #080c14; --bg-subtle: #111827; --text-primary: #e5e9f0; --text-secondary: #8892a6;
  --text-tertiary: #5c6a7e; --accent: #60a5fa; --accent-hover: #3b82f6; --accent-light: #18233a;
  --border: #252f3f; --shadow: 0 1px 2px rgba(0,0,0,0.3); --shadow-card: 0 1px 3px rgba(0,0,0,0.4), 0 1px 2px rgba(0,0,0,0.3);
}}
[data-theme="dark"] .carousel-dot {{ background: #3a4660; }}
[data-theme="dark"] .carousel-btn:hover {{ border-color: var(--accent); color: var(--accent); }}
html {{ scroll-behavior: smooth; }}
body, .sidebar, .main, .footer {{ transition: background 0.4s ease, color 0.4s ease, border-color 0.4s ease; }}
body {{
...
.theme-toggle {{ display: inline-flex; align-items: center; justify-content: center; background: none; border: 1px solid var(--border); border-radius: var(--radius-sm); color: var(--text-tertiary); cursor: pointer; padding: 0.3rem; margin-top: 0.5rem; transition: all var(--transition); }}
...
.copy-tooltip {{
  ...
  background: var(--text-primary); color: var(--bg); font-size: 0.68rem; padding: 0.2rem 0.5rem;
...
@media (max-width: 1023px) {{
  ...
  .sidebar .tagline, .sidebar .info-line, .sidebar .sidebar-section-title, .sidebar .sidebar-experience, .sidebar .email-row, .sidebar .social-links, .sidebar .resume-btn, .sidebar .sidebar-link, .sidebar .theme-toggle {{ display: none; }}
  ...
  .sidebar.expanded .tagline, .sidebar.expanded .info-line, .sidebar.expanded .sidebar-section-title, .sidebar.expanded .sidebar-experience, .sidebar.expanded .email-row, .sidebar.expanded .social-links, .sidebar.expanded .resume-btn, .sidebar.expanded .sidebar-link, .sidebar.expanded .theme-toggle {{ display: flex; }}
...
@media print {{
  ...
  .sidebar {{ position: static; width: 100%; min-height: auto; border: none; padding: 1rem 0; }}
  .sidebar-inner {{ flex-direction: row; flex-wrap: wrap; gap: 0.5rem 1rem; }}
  .avatar {{ width: 40px; height: 40px; font-size: 0.8rem; }}
  .sidebar h1 {{ font-size: 1.1rem; }}
  .sidebar .tagline, .sidebar .info-line, .sidebar .sidebar-section-title, .sidebar .sidebar-experience, .sidebar .email-row, .sidebar .social-links, .sidebar .resume-btn, .sidebar .sidebar-link, .sidebar .theme-toggle, .sidebar-mobile-content, .hamburger {{ display: flex !important; }}
  .main {{ margin: 0; padding: 1rem 0; max-width: 100%; }}
...
<aside class="sidebar" id="sidebar">
  ...
  <div class="sidebar-inner" id="sidebarInner">
    ...
    <div class="social-links">
      ...
      <a href="https://devpost.com/ThePeeps191" target="_blank" rel="noopener" class="social-link">
        <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="3"/><polyline points="12 7 12 17"/><line x1="8" y1="12" x2="16" y2="12"/></svg>
        Devpost
      </a>
    ...
    <button id="theme-toggle" class="theme-toggle" aria-label="Toggle dark mode">
      <svg class="theme-toggle-icon" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>
    </button>
  </div>
</aside>
